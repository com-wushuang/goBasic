...
- 而这个设备的信息，正是每台宿主机上的 `flanneld` 进程负责维护的，当 `Node 2` 启动并加入 `Flannel` 网络之后，在 `Node 1`（以及所有其他节点）上，`flanneld` 就会添加一条如下所示的路由规则：
```shell
$ route -n
...
**封包和解包**
- `Node 1` 和 `Node 2` 上的 `flannel.1` 设备分别称为"源 `VTEP` 设备"和"目的 `VTEP` 设备"。而这些 `VTEP` 设备之间，就需要想办法组成一个虚拟的二层网络，即：通过二层数据帧进行通信。
...
- `Linux` 内核还需要再把"内部数据帧"进一步封装成为宿主机网络里的一个普通的数据帧，好让它载着"内部数据帧"，通过宿主机的 `eth0` 网卡进行传输。
...
- 跟 `UDP` 模式类似，在宿主机看来，它会以为自己的 `flannel.1` 设备只是在向另外一台宿主机的 `flannel.1` 设备，发起了一次普通的 `UDP` 链接。它不会知道，这个 `UDP` 包里面，其实是一个完整的二层数据帧。
- 一个 `flannel.1` 设备只知道另一端的 `flannel.1` 设备的 `MAC` 地址，却不知道对应的宿主机地址是什么，也就是说，这个 UDP 包该发给哪台宿主机呢？
...
- `Linux` 内核再在这个 `IP` 包前面加上二层数据帧头，即原理图中的 `Outer Ethernet Header`，并把 `Node 2` 的 `MAC` 地址填进去。这个 `MAC` 地址本身，是 `Node 1` 的 `ARP` 表要学习的内容，无需 `Flannel` 维护。封装出来的"外部数据帧"的格式，如下所示：
![upd_package](https://github.com/com-wushuang/goBasic/blob/main/image/upd_package.webp)
- 这样，封包工作就宣告完成了。
...
**总结**
- `Flannel UDP` 和 `VXLAN` 模式的工作原理。这两种模式其实都可以称作"隧道"机制，也是很多其他容器网络插件的基础。
